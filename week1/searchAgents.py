        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if (nextx, nexty) in self.corners and (nextx, nexty) not in state[1]:
                    nextState = ( (nextx, nexty), ( nextState[1] + ((nextx, nexty),) ))
                cost = 1
                successors.append( ( nextState, action, cost ) )
...
def cornersHeuristic(state, problem):
    ...
    unvisited_corners = [corner for corner in corners if not corner in state[1] ]
    num_corners_remaining = len(unvisited_corners)
    heuristic = 0

    def md( dest, source=state[0] ):
        return abs(dest[0]-source[0]) + abs(dest[1]-source[1])
   
    def nearest_dot( listOfDots, start=state[0] ):
        return min( [ mazeDistance( start, dot, problem.gs) for dot in listOfDots ])

    if not num_corners_remaining:
        return heuristic
    if num_corners_remaining == 2:
        heuristic = min(mazeDistance(state[0], unvisited_corners[0], problem.gs) +
                        mazeDistance(unvisited_corners[0], unvisited_corners[1], problem.gs),
                        mazeDistance(state[0], unvisited_corners[1], problem.gs) +
                        mazeDistance(unvisited_corners[1], unvisited_corners[0], problem.gs))
    else:
        heuristic = nearest_dot( unvisited_corners )
