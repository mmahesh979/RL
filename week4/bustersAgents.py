...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        likely_ghost_positions = list()
        for ghost_dist in livingGhostPositionDistributions:
            current_most_likely = None      
            current_best_prob = 0
            for position in ghost_dist:
                if(current_best_prob < ghost_dist[position]):
                    current_best_prob = ghost_dist[position]
                    current_most_likely = position
            likely_ghost_positions.append(current_most_likely)

        current_best_move = legal[0]
        current_best_distance = self.distancer.getDistance(pacmanPosition, likely_ghost_positions[0])
        for action in legal: 
            new_position = Actions.getSuccessor(pacmanPosition, action)
            action_distance = min([ self.distancer.getDistance(new_position, pos2) for pos2 in likely_ghost_positions ])
            if( action_distance < current_best_distance ):
                current_best_move = action
                current_best_distance = action_distance
