        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
            ...
            for new_position, prob in newPosDist.items():
                all_possible[new_position] += current_beliefs[position] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = list()
        num_board_positions = len(self.legalPositions)
        particle_every_x_positions = max(1, num_board_positions/self.numParticles)
        num_particles_remaining = self.numParticles

        while num_particles_remaining:
            for i, pos in enumerate(self.legalPositions):
                if not num_particles_remaining:
                    break
                if (i+1)%particle_every_x_positions == 0 and num_particles_remaining:
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.particles:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            if emissionModel[trueDistance] > 0:
                all_possible[p] = emissionModel[trueDistance]*bd[p]
     
        if not all_possible.totalCount():
            self.initializeUniformly(gameState)
            return None

        for i in range(self.numParticles):
            resampled_list.append(util.sample(all_possible))
        self.particles = resampled_list            
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ghost_distribution = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            resampled_list.append(util.sample(ghost_distribution))
        self.particles = resampled_list
    ...
    def getBeliefDistribution(self):
        ...
        """
        out_counter = util.Counter()
        for item in self.particles:
            out_counter[item] += 1.0/len(self.particles)
        out_counter.normalize()
        return out_counter
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # Enumerate a list of valid positions for each ghost separately. 
        ghost1_positions = list(self.legalPositions)
        ghost2_positions = list(self.legalPositions)
    
        # Take the product of these two lists and make a list of those products. 
        ghost_cross_positions = itertools.product(ghost1_positions, ghost2_positions)
        ghost_list = [position for position in ghost_cross_positions]

        #def is_unique(t1, t2):
        #    for element in t2:
        #        if not element in t1:
        #            return True
        #    for element in t1:
        #        if not element in t2:
        #            return True
        #    return False
        #for ghosts in temp_list:
        #    okay_to_add = True
        #    for elem in ghost_list:
        #        if not is_unique(ghosts, elem):
        #            okay_to_add = False
        #    if okay_to_add:
        #        ghost_list.append(ghosts)

        self.particles = list()
        random.shuffle(ghost_list)
        for i in range(self.numParticles):
            self.particles.append(ghost_list[i%len(ghost_list)])
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                for p_index in range(self.numParticles):
                    self.particles[p_index] = self.getParticleWithGhostInJail(self.particles[p_index], i)

        bd = self.getBeliefDistribution()
        for ghosts in self.particles:
            working_value = 1.0
            for i in range(self.numGhosts):
                if ghosts[i] == self.getJailPosition(i):
                    continue
                trueDistance = util.manhattanDistance(ghosts[i], pacmanPosition)
                working_value *= emissionModels[i][trueDistance]
            all_possible[ghosts] = working_value*bd[ghosts]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            #for e,particle in enumerate(newParticle):
            for i in range(self.numGhosts):      
    ...
    def getBeliefDistribution(self):  
        out_counter = util.Counter()
        for item in self.particles:
            out_counter[item] += 1.0/self.numParticles
        out_counter.normalize()
        return out_counter 
